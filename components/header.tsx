...
function Header() {
  // z-index does not work with fixed position

  const ctx = useActiveSectionContext();
            ...
            >
              <Link
                className={`flex w-full items-center justify-center px-3 py-3 hover:text-gray-950 transition dark:text-gray-500 dark:hover:text-gray-300 ${
                  ctx?.activeSection === link.name
                    ? "text-gray-950  dark:text-gray-200"
                    : ""
                }  `}
                href={link.hash}
                onClick={() => {
                  ctx?.setActiveSection(link.name);
                  ctx?.setTimeOfLastClick(Date.now());
                }}
              >
                {link.name === ctx?.activeSection && (
                  <motion.span
